...
pub struct MakeAccounts<'info> {
    ...
    // 注意：Pinocchio 版本不需要 associated_token_program 账户
    // 对应 Anchor: pub associated_token_program: Program<'info, AssociatedToken>
    //
    // Anchor 的隐式使用：
    // - 在账户验证阶段（Context 创建时），Anchor 检测到 vault 账户的
    //   init + associated_token:: 约束
    // - 宏自动生成的代码会调用 associated_token_program 来创建 ATA
    // - 这个过程在 handler 函数执行之前完成，完全透明
    //
    // Pinocchio 的处理方式：
    // - AssociatedTokenAccount::init 方法内部使用硬编码的
    ...
    // - ❌ 无法切换到自定义的 ATA 程序实现
    // - ❌ 依赖硬编码的程序地址
    // - Anchor: 更灵活，支持自定义 ATA 程序（通过传入不同的 program 账户）
    // - Pinocchio: 更简洁，但仅支持标准 ATA 程序
}
...
impl<'info> TryFrom<&'info [AccountView]> for MakeAccounts<'info> {
    ...
    fn try_from(accounts: &'info [AccountView]) -> Result<Self, Self::Error> {
        ...
        // *** 重要：这些账户来自客户端构建的交易 ***
        //
        // 客户端端（TypeScript/JavaScript）：
        // const transaction = new Transaction().add(
        //   new TransactionInstruction({
        //     keys: [
        //       {pubkey: makerKey, isSigner: true, isWritable: true},
        //       {pubkey: escrowKey, isSigner: false, isWritable: true},  // ← 客户端计算的 PDA
        //       {pubkey: mintA, isSigner: false, isWritable: false},
        //       {pubkey: mintB, isSigner: false, isWritable: false},
        //       {pubkey: makerAtaA, isSigner: false, isWritable: true},
        //       {pubkey: vaultKey, isSigner: false, isWritable: true},   // ← 客户端计算的 PDA
        ...
        // );
        //
        // Solana 运行时将这些账户传递给程序的 entrypoint
        // 然后按顺序解构到对应的变量
        ...
        // *** PDA 验证机制（关键！）***:
        //
        // 虽然这里没有显式验证 escrow 地址是否正确，但验证通过后续的
        // invoke_signed 隐式完成（在 make.rs:390 的 ProgramAccount::init 中）：
        //
        // System Program 的 invoke_signed 会验证：
        // 1. 使用程序计算的签名种子重新计算 PDA
        // 2. 验证计算出的地址 == 客户端传入的 accounts.escrow
        // 3. 如果不匹配，交易失败（InvalidSignature）
        //
        // 这样确保客户端无法传入错误的 escrow 地址！
        // vault 的验证同理（使用 ATA 的验证机制）。
...
impl<'info> TryFrom<(&'info [u8], &'info [AccountView])> for Make<'info> {
    ...
    fn try_from((data, accounts): (&'info [u8], &'info [AccountView])) -> Result<Self, Self::Error> {
        ...
        //
        // Anchor 版本（make_anchor.rs:101-116）：
        //   #[account(
        //       init,                          // ← 触发 ATA 创建
        //       payer = maker,                 // ← maker 支付费用
        //       associated_token::mint = mint_a,
        ...
        //   )]
        //   pub vault: InterfaceAccount<'info, TokenAccount>,
        //   pub associated_token_program: Program<'info, AssociatedToken>,  // ← 隐式使用
        //
        // Anchor 的隐式调用机制：
        // 1. Context<Make> 创建时（验证阶段）
        // 2. Anchor 检测到 init + associated_token:: 约束
        // 3. 宏自动生成的代码调用 associated_token_program
        // 4. ATA 创建完成后才进入 handler 函数
        // 5. 整个过程对开发者透明，无需显式调用
        //
        // Pinocchio 手动实现：
        // 必须显式调用 Associated Token Account Program 创建 ATA

        // 创建金库 ATA
        // 对应 Anchor 的验证阶段自动调用 ATA Program
        //
        // Pinocchio 与 Anchor 的关键差异：
        //
        // 1. 调用时机：
        //    - Anchor: 验证阶段（Context 创建时），宏自动处理
        //    - Pinocchio: try_from 中手动调用
        //
        // 2. associated_token_program 账户：
        //    - Anchor: 必须传入，供宏在验证阶段使用
        //    - Pinocchio: 不需要传入，使用硬编码的程序地址
        //
        // 3. helpers.rs 中的 AssociatedTokenAccount::init 实现细节：
        //    - 通过 CPI 调用 Associated Token Account Program
        //    - 使用固定的程序 ID：ATA_TOKEN_PROGRAM_ID
        //    - 不需要客户端传入该程序账户
        //
        // 4. 灵活性权衡：
        //    - Anchor: 更灵活，支持自定义 ATA 程序（通过传入不同的 program 账户）
